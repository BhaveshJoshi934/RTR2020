...
static GLfloat delay_texture_bdj = 30.0f;

//For Texture code

GLuint Day_Texture;
GLuint DayTwo_Texture;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szAppName, TEXT("Textures!!!"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, X, Y, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
...
void Initialize(void)
{
	//Function Declaration
	void Resize(int, int);
	bool LoadGLTexture(GLuint*,TCHAR[]);      //Texture Code
	...
	if(delay_texture_bdj <= 30.0f)
    {
        LoadGLTexture(&Day_Texture,MAKEINTRESOURCE(DAY_BITMAP));
        glEnable(GL_TEXTURE_2D);
    }

    if(delay_texture_bdj <= 20.0f)
    {
        LoadGLTexture(&DayTwo_Texture,MAKEINTRESOURCE(DAYTWO_BITMAP));
        glEnable(GL_TEXTURE_2D);
    }
    if(delay_texture_bdj <= 10.0f)
    {
...
void Display(void)
{
	...
	if(delay_texture_bdj <= 20.0f)
    {
        glBindTexture(GL_TEXTURE_2D,DayTwo_Texture);
	...
	delay_texture_bdj = delay_texture_bdj - 0.01f;
...
void UnInitialize(void)
{
	...
	glDeleteTextures(1,&DayTwo_Texture);
