...
void LaunchCPUKernel(unsigned int,unsigned int,GLfloat);
...
float pos[mesh_height * mesh_width * 4];
...
void Initialize()
{
    ...
    for(i = 0 ; i < mesh_width ; i++)
    {
        for(j = 0 ; j < mesh_height ; j++)
        {
            for(k = 0 ; k < 4 ; k++)
            {
                pos[mesh_height * i * 4 + j * 4 + k] = 0.0f;
            }
        }
    }
/*
...
*/
    ...
    glVertexAttribPointer(BDJ_ATTRIBUTE_POSITION,4,GL_FLOAT,GL_FALSE,0,NULL);
...
void LaunchCPUKernel(unsigned int width,unsigned int height,GLfloat time)
{
    for(i = 0 ; i < width ; i++)
    {
        for(j = 0 ; j < height ; j++)
        {
            for(k = 0 ; k < 4 ; k++)
            {
                ...
                if(k == 0)
                {
                    pos[width * i * 4 + j * 4 + k] = u;
                }
                if(k == 1)
                {
                    pos[width * i * 4 + j * 4 + k] = w;
                }
                if(k == 2)
                {
                    pos[width * i * 4 + j * 4 + k] = v;
                }
                if(k == 3)
                {
                    pos[width * i * 4 + j * 4 + k] = 1.0f;
                }
            }
        }
    }
...
void Display()
{
    ...
    else
    {
        ...
        glBindBuffer(GL_ARRAY_BUFFER,vbo_Position);
        glBufferData(GL_ARRAY_BUFFER,MyArraySize * sizeof(GLfloat),pos,GL_DYNAMIC_DRAW);
    }
...
cl.exe /c /EHsc /I C:\glew-2.1.0-win32\glew-2.1.0\include /I "CUDA PATH" MyWindow.cpp

link.exe MyWindow.obj sinewave.cu.obj user32.lib gdi32.lib /LIBPATH:"cuda path" /LIBPATH:C:\glew-2.1.0-win32\glew-2.1.0\lib\Release\Win32 /MACHINE:X64 /SUBSYSTEM:WINDOWS
*/
